...
class GridWorldGUI:
    ...
    def add_dynamic_obstacle(self):
        """添加动态障碍物"""
        count = self.dynamic_obstacle_count.get()
        # 重新创建环境以添加动态障碍物
        self.env.add_dynamic_obstacles(count)
        self.draw_grid()
        self.status_label.config(text=f"已添加 {count} 个动态障碍物")
    ...
    def run_reinforcement_learning(self):
        ...
        try:
            ...
            # 创建智能体
            q_agent = QLearningAgent(self.env)
            sarsa_agent = SarsaAgent(self.env)
            pg_agent = PolicyGradientAgent(self.env)
            
            # 训练智能体
            self.status_label.config(text="正在训练Q-Learning智能体...")
            self.root.update()
            q_rewards = q_agent.train(episodes=500)
            
            self.status_label.config(text="正在训练SARSA智能体...")
            self.root.update()
            sarsa_rewards = sarsa_agent.train(episodes=500)
            
            self.status_label.config(text="正在训练Policy Gradient智能体...")
            self.root.update()
            pg_rewards = pg_agent.train(episodes=500)
            ...
            # 显示结果
            self.status_label.config(text="训练完成，正在显示结果...")
            self.show_results(q_path, sarsa_path, pg_path, q_rewards, sarsa_rewards, pg_rewards)
            
        except Exception as e:
            messagebox.showerror("运行失败", f"无法运行强化学习算法: {str(e)}")
        
    def show_results(self, q_path, sarsa_path, pg_path, q_rewards, sarsa_rewards, pg_rewards):
        """显示强化学习结果"""
        # 这里可以显示结果图表或更新GUI
        messagebox.showinfo("训练完成", 
                           f"强化学习训练已完成!\n"
                           f"Q-Learning最终奖励: {q_rewards[-1]}\n"
                           f"SARSA最终奖励: {sarsa_rewards[-1]}\n"
                           f"Policy Gradient最终奖励: {pg_rewards[-1]}")
    ...
    def on_grid_click(self, event):
        ...
        if mode == "start":
            ...
            # 设置动态障碍物轨迹点（这里简化处理，实际应用中可以更复杂）
            self.status_label.config(text=f"动态障碍物轨迹点 ({x}, {y})")
